...
//     // For each user, fetch the latest wallet transaction balance
//     const userRecordsWithBalance = await Promise.all(
//       userRecords.map(async (user) => {
//         // Fetch the latest wallet transaction for the user
//         const latestTransaction = await WalletTransaction.findOne({
//           userId: user._id,
//         })
//           .sort({ createdAt: -1 }) // Sort by creation date to get the latest transaction
//           .select("balance"); // Select only the balance field

//         return {
//           ...user.toObject(),
//           walletBalance: latestTransaction ? latestTransaction.balance : 0, // Use latest transaction balance or 0 if no transactions
//         };
//       })
//     );
...
//         return {
//           ...user.toObject(),
//           walletBalance: wallet ? wallet.walletAmount : 0,
//         };
//       })
//     );
