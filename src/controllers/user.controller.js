...
const registerUser = asyncHandler(async (req, res) => {
	try {
		...
		return res.status(201).json(new ApiResponse(200, { createdUser, walletDetails }, "User registered Successfully"));
	} catch (error) {
...
const loginUser = asyncHandler(async (req, res) => {
	...
	const walletDetails = allWalletDetail?.find((itm => itm.userId === loggedInUser.userId));
	...
	return res
		...
		.json(new ApiResponse(200, { user: loggedInUser, data: plans, walletDetails: walletDetails, accessToken, refreshToken }, "User logged In Successfully"));
});
...
// Controller to get users at different referral levels
const getUsersAtLevel = asyncHandler(async (req, res, _, user_id = null, visitedUsers = null) => {
	try {
		// Extract user_id from query if not passed as an argument
		const userId = user_id || req?.query?.user_id;
...
const getUsersLevel = async (userId, visitedUsers = new Set()) => {
	try {
		...
		for (let referral of directReferrals) {
			const deeperReferrals = await getUsersLevel(referral.userId, visitedUsers);
...
const getWalletAddress = asyncHandler(async (req, res) => {
	const addresses = await addressSchemaWithdrawal.find(); // You can modify the query as per your needs
