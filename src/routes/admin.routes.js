...
router.route('/updateProduct').post(verifyJWT, upload.fields([{
	name: "productImg",
	maxCount: 1
}]),
	updateProduct);
...
//route add slider
router.route("/addslider").post(verifyJWT, upload.fields([{
	name: "sliderImg",
	maxCount: 1
}]), uploadSliderImage);
// Route to get slider
...
router.route('/addPlan').post(verifyJWT, upload.fields([{
	name: "planImg",
	maxCount: 1
}]), addPlan);
// Route to get all event records, protected by JWT verification
...
// Route to get all event records, protected by JWT verification
router.route('/updatePlan').post(verifyJWT, upload.fields([{
	name: "planImg",
	maxCount: 1
}]), updatePlan);
...
// router.route('sendMessage/:id').post(verifyJWT, sendAdminMessage); // Admin sends message by passing admin ID
// router.route('/sendMessage/:id').post(verifyJWT, upload.fields([{
// 	name: "chatImg",
// 	maxCount: 1
// }]), sendAdminMessage);
// router.route('/getMessages/:id').get(verifyJWT, getAdminMessages); // Admin fetches messages by admin ID
// router.route('/deleteMessage/:id').delete(verifyJWT, deleteAdminMessage); // Admin deletes messages by admin ID
